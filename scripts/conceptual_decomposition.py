...
def process_all(texts: list[object], client: OpenAI, model_config: dict, temperature: float, top_p: float, request_interval: float, max_workers: int, retries: int) -> list[tuple[str, Optional[float], str]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(classify_text, text, client, model_config, temperature, top_p, request_interval, retries): idx for idx, text in enumerate(texts)}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run conceptual decomposition.')
