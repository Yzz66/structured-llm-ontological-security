...
MODEL_PRESETS = {'llama-3.1-8b': ModelConfig(model_name='meta/llama-3.1-8b-instruct', base_url='https://integrate.api.nvidia.com/v1', api_key_env='NVIDIA_API_KEY', disable_thinking=False), 'llama-3.1-70b': ModelConfig(model_name='meta/llama-3.1-70b-instruct', base_url='https://integrate.api.nvidia.com/v1', api_key_env='NVIDIA_API_KEY', disable_thinking=False), 'qwen3-32b': ModelConfig(model_name='qwen3-32b', base_url='https://dashscope.aliyuncs.com/compatible-mode/v1', api_key_env='DASHSCOPE_API_KEY', disable_thinking=True)}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run 20-shot LLM prompting for ontological security narrative classification.')
    ...
    parser.add_argument('--model-preset', choices=sorted(MODEL_PRESETS), default='llama-3.1-70b', help='Model preset to use.')
    ...
    parser.add_argument('--request-interval', type=float, default=DEFAULT_REQUEST_INTERVAL, help='Minimum interval in seconds between API calls.')
    parser.add_argument('--retries', type=int, default=3, help='Number of API retries.')
    parser.add_argument('--disable-thinking', action='store_true', help="Pass extra_body={'enable_thinking': False}; useful for Qwen models.")
...
def resolve_model_config(args: argparse.Namespace) -> ModelConfig:
    ...
    return ModelConfig(model_name=args.model_name or preset.model_name, base_url=args.base_url or preset.base_url, api_key_env=args.api_key_env or preset.api_key_env, disable_thinking=args.disable_thinking or preset.disable_thinking)

def build_client(config: ModelConfig) -> OpenAI:
    api_key = os.getenv(config.api_key_env)
    if not api_key:
        raise RuntimeError(f'Missing API key. Please set the environment variable {config.api_key_env}.')
...
def parse_prediction(raw: str) -> tuple[str, Optional[float]]:
    ...
    cleaned = raw.strip()
    label_match = re.search('(?:Form|Primary)?\\s*:?\\s*<?\\s*(Pride|Shame|Insult|Denial|None)\\s*>?', cleaned, re.IGNORECASE)
    if label_match:
        label = label_match.group(1).capitalize()
    else:
        head_match = re.match('\\s*(Pride|Shame|Insult|Denial|None)\\b', cleaned, re.IGNORECASE)
        if head_match:
            label = head_match.group(1).capitalize()
    confidence_match = re.search('confidence\\s*:\\s*(\\d{1,3})\\s*%', cleaned, re.IGNORECASE)
    if not confidence_match:
        confidence_match = re.search('(\\d{1,3})\\s*%', cleaned)
    if confidence_match:
...
def call_model(client: OpenAI, config: ModelConfig, prompt: str, temperature: float, top_p: float, request_interval: float, retries: int) -> str:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            kwargs = {'model': config.model_name, 'messages': [{'role': 'user', 'content': prompt}], 'temperature': temperature, 'top_p': top_p}
...
def classify_primary(text, client: OpenAI, config: ModelConfig, args: argparse.Namespace) -> tuple[str, Optional[float], str]:
    ...
    raw = call_model(client=client, config=config, prompt=prompt, temperature=args.temperature, top_p=args.top_p, request_interval=args.request_interval, retries=args.retries)
    if raw == 'API Error':
...
def process_all(texts: list[str], client: OpenAI, config: ModelConfig, args: argparse.Namespace) -> list[tuple[str, Optional[float], str]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = {executor.submit(classify_primary, text, client, config, args): i for i, text in enumerate(texts)}
...
def compute_metrics(y_true: Iterable[str], y_pred: Iterable[str]) -> dict:
    ...
    try:
        alpha5 = krippendorff.alpha(reliability_data=[y_true_norm, y_pred_norm], level_of_measurement='nominal')
    except Exception:
        alpha5 = float('nan')
    ...
    if y_pred_covered:
        ...
        kappa4 = float('nan')
        acc4 = float('nan')
    return {'cm5': cm5, 'kappa5': kappa5, 'alpha5': alpha5, 'acc5': acc5, 'coverage': coverage, 'kappa4_cov': kappa4, 'acc4_cov': acc4, 'n': len(y_true_norm), 'n_cov': len(y_pred_covered)}
...
def write_evaluation_report(eval_path: Path, args: argparse.Namespace, config: ModelConfig, df_run: pd.DataFrame, full_df: pd.DataFrame) -> None:
    ...
    with eval_path.open('w', encoding='utf-8') as f:
        ...
        for run_id in range(1, args.n_runs + 1):
            ...
            f.write('=' * 70 + '\n')
            f.write(f"kappa5={metrics['kappa5']:.4f}, alpha5={metrics['alpha5']:.4f}, acc5={metrics['acc5']:.4f}, coverage={metrics['coverage']:.4f} ({metrics['n_cov']}/{metrics['n']}), kappa4_cov={metrics['kappa4_cov']:.4f}, acc4_cov={metrics['acc4_cov']:.4f}\n\n")
        f.write('\n' + '=' * 70 + '\n')
        ...
        f.write('=' * 70 + '\n')
        f.write(classification_report([normalize_label(value) for value in y_true_all], [normalize_label(value) for value in y_pred_all], labels=EVAL_LABELS, zero_division=0))
        ...
        if y_pred_covered:
            ...
            f.write('=' * 70 + '\n')
            f.write(classification_report(y_true_covered, y_pred_covered, labels=EVAL_LABELS_4, zero_division=0))
