...
def build_pipeline(seed):
    return Pipeline([('tfidf', TfidfVectorizer(ngram_range=TFIDF_NGRAM_RANGE, min_df=TFIDF_MIN_DF, max_df=TFIDF_MAX_DF)), ('clf', RandomForestClassifier(n_estimators=N_ESTIMATORS, random_state=seed, max_depth=MAX_DEPTH, min_samples_leaf=MIN_SAMPLES_LEAF, n_jobs=N_JOBS))])
...
def main():
    ...
    txt_path = output_dir / f'RF_train_test_results_{timestamp}.txt'
    with txt_path.open('w', encoding='utf-8') as f:
        f.write('Random Forest Train/Test Evaluation\n')
        f.write('=' * 70 + '\n\n')
        ...
        f.write(f'RandomForest: n_estimators={N_ESTIMATORS}, max_depth={MAX_DEPTH}, min_samples_leaf={MIN_SAMPLES_LEAF}, n_jobs={N_JOBS}\n\n')
        f.write('[Summary]\n')
        f.write('-' * 70 + '\n')
        ...
        for run_id, report in reports.items():
            ...
            f.write(report)
            f.write('\n')
    pred_path = output_dir / f'RF_train_test_predictions_{timestamp}.xlsx'
    metrics_path = output_dir / f'RF_train_test_metrics_{timestamp}.xlsx'
