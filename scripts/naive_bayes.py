...
def get_label_order(y_train, y_test):
    return sorted(set(y_train) | set(y_test))

def build_pipeline():
    return Pipeline([('tfidf', TfidfVectorizer(ngram_range=TFIDF_NGRAM_RANGE, min_df=TFIDF_MIN_DF, max_df=TFIDF_MAX_DF, sublinear_tf=TFIDF_SUBLINEAR_TF)), ('clf', MultinomialNB(alpha=ALPHA))])
...
def main():
    ...
    report = classification_report(y_test, final_pred, labels=labels, digits=4, zero_division=0)
    ...
    txt_path = output_dir / f'Bayes_train_test_results_{timestamp}.txt'
    with txt_path.open('w', encoding='utf-8') as f:
        f.write('Naive Bayes Train/Test Evaluation\n')
        f.write('=' * 70 + '\n\n')
        ...
        f.write(f'TF-IDF: ngram_range={TFIDF_NGRAM_RANGE}, min_df={TFIDF_MIN_DF}, max_df={TFIDF_MAX_DF}, sublinear_tf={TFIDF_SUBLINEAR_TF}\n')
        f.write(f'MultinomialNB: alpha={ALPHA}\n\n')
        f.write('[Summary]\n')
        f.write('-' * 70 + '\n')
        ...
        f.write('\n\n[Confusion Matrix: Final Run]\n')
        f.write('-' * 70 + '\n')
        f.write('Rows = true labels, columns = predicted labels\n')
        f.write(format_confusion_matrix(cm, labels))
    ...
    pred_path = output_dir / f'Bayes_train_test_predictions_{timestamp}.xlsx'
    metrics_path = output_dir / f'Bayes_train_test_metrics_{timestamp}.xlsx'
    ...
    with pd.ExcelWriter(metrics_path, engine='openpyxl') as writer:
        ...
        pd.DataFrame(cm, index=labels, columns=labels).to_excel(writer, sheet_name='confusion_matrix_final')
