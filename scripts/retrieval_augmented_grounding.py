...
def resolve_model_config(args: argparse.Namespace) -> dict:
    ...
    return {'model': model_name, 'base_url': base_url, 'api_key_env': api_key_env, 'api_key': api_key, 'disable_thinking': disable_thinking, 'preset_name': args.model_preset}
...
def chat_completion(client: OpenAI, model_name: str, prompt: str, temperature: float, top_p: float, request_interval: float, disable_thinking: bool):
...
def parse_layer1_confidence_output(raw_output: str) -> dict:
    if not raw_output or not isinstance(raw_output, str):
        return {'mode': 'confidence', 'primary': 'Unknown', 'secondary': None, 'confidence': None, 'candidates': [], 'raw': raw_output}
    text = raw_output.strip()
    primary = 'Unknown'
    for form in VALID_FORMS:
        if re.search(f'\\b{form}\\b', text, re.IGNORECASE):
            primary = form
            break
    confidence = None
    percent_match = re.search('(\\d{1,3})\\s*%', text)
    if percent_match:
        confidence = int(percent_match.group(1)) / 100.0
    else:
        float_match = re.search('\\b(0\\.\\d+)\\b', text)
        if float_match:
            confidence = float(float_match.group(1))
    return {'mode': 'confidence', 'primary': primary, 'secondary': None, 'confidence': confidence, 'candidates': [primary] if primary != 'Unknown' else [], 'raw': raw_output}

def build_layer2_global_query() -> str:
    return 'Pride Denial Shame Insult disavow or to disclaim awareness offset fear and shame narrative responsibility agency self other difference contrast vs'
...
def parse_layer3_confidence_output(raw_output: str) -> tuple[Optional[str], Optional[float]]:
    if not raw_output or not isinstance(raw_output, str):
        return (None, None)
    text = raw_output.strip()
    strict_match = re.search('(?:Final|PRIMARY|Primary)\\s*[:=]\\s*(Pride|Shame|Denial|Insult|None)\\s*\\(\\s*confidence\\s*[:=]?\\s*(\\d{1,3})\\s*%\\s*\\)', text, re.IGNORECASE)
    if strict_match:
        label = strict_match.group(1).capitalize()
        score = min(100, max(0, int(strict_match.group(2))))
        return (label, score / 100.0)
    clean = re.sub('\\*+', '', text)
    clean_match = re.search('(?:Final|PRIMARY|Primary)\\s*[:=]\\s*(Pride|Shame|Denial|Insult|None)\\s*\\(\\s*confidence\\s*[:=]?\\s*(\\d{1,3})\\s*%\\s*\\)', clean, re.IGNORECASE)
    if clean_match:
        label = clean_match.group(1).capitalize()
        score = min(100, max(0, int(clean_match.group(2))))
        return (label, score / 100.0)
    type_match = re.search('\\b(Pride|Shame|Denial|Insult|None)\\b', clean, re.IGNORECASE)
    conf_match = re.search('(\\d{1,3})\\s*%', clean)
...
def compute_metrics(y_true: list[object], y_pred: list[object]) -> dict:
    ...
    return {'cm5': cm5, 'kappa5': kappa5, 'alpha5': alpha5, 'acc5': acc5, 'coverage': coverage, 'kappa4_cov': kappa4, 'acc4_cov': acc4, 'n': len(y_true_n), 'n_cov': len(y_pred_cov)}
...
def write_confidence_mode_summary_multi(df: pd.DataFrame, label_col: str, summary_path: Path, input_file: Path, n_runs: int) -> None:
    ...
    with summary_path.open('w', encoding='utf-8') as file:
        ...
        for run_idx in range(1, n_runs + 1):
            ...
            file.write('=' * 60 + '\n')
            file.write(f"[Layer1] kappa5={layer1_metrics['kappa5']:.4f}, alpha5={layer1_metrics['alpha5']:.4f}, acc5={layer1_metrics['acc5']:.4f}, coverage={layer1_metrics['coverage']:.4f}, kappa4_cov={layer1_metrics['kappa4_cov']:.4f}, acc4_cov={layer1_metrics['acc4_cov']:.4f}\n")
            file.write(f"[Final ] kappa5={final_metrics['kappa5']:.4f}, alpha5={final_metrics['alpha5']:.4f}, acc5={final_metrics['acc5']:.4f}, coverage={final_metrics['coverage']:.4f} ({final_metrics['n_cov']}/{final_metrics['n']}), kappa4_cov={final_metrics['kappa4_cov']:.4f}, acc4_cov={final_metrics['acc4_cov']:.4f}\n")
        ...
        if y_pred_cov:
            ...
            file.write('=' * 60 + '\n')
            report_4 = classification_report(y_true_cov, y_pred_cov, labels=EVAL_LABELS_4, zero_division=0)
            file.write(report_4)
...
def run_pipeline(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    layer2_scores = ','.join((f"{hit['score']:.3f}" for hit in hits if hit['score'] >= args.similarity_log_threshold))
    ...
    for row_number, (row_index, row) in enumerate(df.iterrows(), start=1):
        ...
        for run_idx in range(1, args.n_runs + 1):
            ...
            layer1_response = chat_completion(client=client, model_name=model_name, prompt=build_layer1_confidence_prompt(text), temperature=args.temperature, top_p=args.top_p, request_interval=args.request_interval, disable_thinking=model_config['disable_thinking'])
            ...
            if not enter_layer2 or not evidence_text.strip():
                ...
                continue
            layer3_response = chat_completion(client=client, model_name=model_name, prompt=build_layer3_confidence_prompt(text=text, evidence=evidence_text), temperature=args.temperature, top_p=args.top_p, request_interval=args.request_interval, disable_thinking=model_config['disable_thinking'])
    ...
    output_stem = f'retrieval_augmented_grounding_threshold_{args.confidence_threshold:g}_temperature_{args.temperature:g}_{safe_name}'
    ...
    if args.label_col and args.label_col in df.columns:
        write_confidence_mode_summary_multi(df=df, label_col=args.label_col, summary_path=summary_file, input_file=input_file, n_runs=args.n_runs)
        print(f'[OK] Evaluation summary saved to: {summary_file}')
    return (output_file, summary_file)

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run Retrieval-augmented grounding with confidence-triggered OSKR retrieval.')
    ...
    parser.add_argument('--model-preset', default='llama-3.1-70b', choices=['llama-3.1-8b', 'llama-3.1-70b', 'qwen3-32b', 'custom'], help='Predefined model/provider configuration.')
    ...
    parser.add_argument('--disable-thinking', action=argparse.BooleanOptionalAction, default=None, help="Whether to pass extra_body={'enable_thinking': False}. Default depends on model preset.")
