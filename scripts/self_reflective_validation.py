...
DEFAULT_TOP_P = 1.0
MODEL_PRESETS = {'llama-3.1-8b': {'model': 'meta/llama-3.1-8b-instruct', 'base_url': 'https://integrate.api.nvidia.com/v1', 'api_key_env': 'NVIDIA_API_KEY', 'disable_thinking': False}, 'llama-3.1-70b': {'model': 'meta/llama-3.1-70b-instruct', 'base_url': 'https://integrate.api.nvidia.com/v1', 'api_key_env': 'NVIDIA_API_KEY', 'disable_thinking': False}, 'qwen3-32b': {'model': 'qwen3-32b', 'base_url': 'https://dashscope.aliyuncs.com/compatible-mode/v1', 'api_key_env': 'DASHSCOPE_API_KEY', 'disable_thinking': True}}
...
def resolve_model_config(args: argparse.Namespace) -> dict:
    if args.model_preset == 'custom':
        preset = {'model': None, 'base_url': os.getenv('OPENAI_BASE_URL'), 'api_key_env': 'OPENAI_API_KEY', 'disable_thinking': False}
    else:
    ...
    if not api_key:
        raise ValueError(f'No API key found. Set {api_key_env} or OPENAI_API_KEY before running this script.')
    return {'model': model, 'base_url': base_url, 'api_key_env': api_key_env, 'api_key': api_key, 'disable_thinking': disable_thinking, 'preset_name': args.model_preset}
...
def chat_completion(client: OpenAI, model_name: str, prompt: str, temperature: float, top_p: float, disable_thinking: bool):
    kwargs = {'model': model_name, 'messages': [{'role': 'user', 'content': prompt}], 'temperature': temperature, 'top_p': top_p}
    if disable_thinking:
...
def parse_label_and_confidence(raw_output: str) -> tuple[str, Optional[float]]:
    label = 'Unknown'
    confidence = None
    patterns = ['Form\\s*:\\s*(Pride|Shame|Denial|Insult|None)', 'Primary\\s*:\\s*(Pride|Shame|Denial|Insult|None)\\s*\\(\\s*(?:confidence\\s*:\\s*)?(\\d+)%\\s*\\)', 'Primary\\s*:\\s*(Pride|Shame|Denial|Insult|None)', '(?:Form|Primary)?\\s*:?\\s*<?\\s*(Pride|Shame|Insult|Denial|None)\\s*>?']
    for pattern in patterns:
        ...
        if match:
            ...
            break
    if label == 'Unknown':
        head_match = re.match('\\s*(Pride|Shame|Denial|Insult)\\b', raw_output, re.IGNORECASE)
        if head_match:
            label = head_match.group(1).capitalize()
    if confidence is None:
        confidence_match = re.search('confidence\\s*:\\s*(\\d+)\\s*%', raw_output, re.IGNORECASE)
        if confidence_match:
...
def filter_by_rank(df: pd.DataFrame, use_rank_filter: bool, rank_values: Optional[list[int]], rank_range: Optional[tuple[int, int]]) -> pd.DataFrame:
...
def compute_metrics(y_true: list[object], y_pred: list[object]) -> dict:
    ...
    try:
        ...
        alpha5 = krippendorff.alpha(reliability_data=[y_true_n, y_pred_n], level_of_measurement='nominal')
    except Exception:
        alpha5 = float('nan')
    ...
    return {'cm5': cm5, 'kappa5': kappa5, 'alpha5': alpha5, 'acc5': acc5, 'coverage': coverage, 'kappa4_cov': kappa4, 'acc4_cov': acc4, 'n': len(y_true_n), 'n_cov': len(y_pred_covered)}
...
def write_evaluation_report(eval_path: Path, method_name: str, input_file: Path, model_config: dict, n_runs: int, temperature: float, y_true: list[object], y_pred_by_run: list[list[object]]) -> None:
    run_metrics = []
    ...
    with open(eval_path, 'w', encoding='utf-8') as file:
        ...
        for run_id, y_pred in enumerate(y_pred_by_run, start=1):
            ...
            file.write('=' * 64 + '\n')
            file.write(f"kappa5={metrics['kappa5']:.4f}, alpha5={metrics['alpha5']:.4f}, acc5={metrics['acc5']:.4f}, coverage={metrics['coverage']:.4f} ({metrics['n_cov']}/{metrics['n']}), kappa4_cov={metrics['kappa4_cov']:.4f}, acc4_cov={metrics['acc4_cov']:.4f}\n")
        file.write('\n' + '=' * 64 + '\n')
...
def add_common_arguments(parser: argparse.ArgumentParser, default_output_dir: str, default_workers: int, default_interval: float) -> None:
    ...
    parser.add_argument('--model-preset', choices=[*MODEL_PRESETS.keys(), 'custom'], default='llama-3.1-70b', help="Model/provider preset. Use 'custom' with --model and optionally --base-url.")
    ...
    parser.add_argument('--disable-thinking', action=argparse.BooleanOptionalAction, default=None, help="Whether to pass extra_body={'enable_thinking': False}. Default follows the selected preset.")
...
METHOD_NAME = 'Self-reflective validation'
OUTPUT_PREFIX = 'self_reflective_validation'
DEFAULT_OUTPUT_DIR = 'outputs/self_reflective_validation'
...
PROMPT_LEVEL2 = "\n[Role]\nPolitical narrative analyst.\n\n[Context]\nThe following text has been initially classified as:\n\nInitial classification: {PRIMARY_L1}\nInitial confidence: {CONF_L1}%\n\n[Task]\nStep back and verify whether the initial classification is theoretically justified.\nAssign the final ontological security narrative to the following text: Pride, Shame, Denial, Insult, or None.\n\n[Analytical Procedure]\n\n1. Identify what the text is responding to, if anything, and how the narrator positions itself in relation to that context.\n2. Examine whether the narrative emphasis lies on rejecting a claim about the narrator, or on characterizing others.\n3. Based on this analysis, assess whether the minimal conditions for {PRIMARY_L1} are satisfied and output the reconsidered classification.\n\n[Theoretical framing]\n\n1. Conduct the analysis strictly from the narrator's perspective.\n2. Ontological security forms are defined as follows:\n- Pride: the narrator positively evaluates their own actions, identity, achievements, or moral standing.\n- Shame: the narrator negatively evaluates their own actions, identity, or responsibility, without rejecting that responsibility.\n- Denial: the narrator denies their own action, statement, involvement, or responsibility.\n- Insult: the narrator uses devaluation of others as the primary narrative purpose.\n\n[Text]\n{CORPUS}\n\n[Output instruction]\n- Output ONLY the final classification and confidence score.\n- Do NOT output any explanation, analysis, reasoning steps, or intermediate thoughts.\n- Provide a confidence score from 0 to 100.\n- Use the following format:\n\nPrimary: <type> (confidence: <score>%)\n\n".strip()

def call_prompt(prompt: str, client: OpenAI, model_config: dict, temperature: float, top_p: float, request_interval: float, retries: int) -> tuple[str, Optional[float], str]:
    for attempt in range(1, retries + 1):
        try:
            ...
            response = chat_completion(client=client, model_name=model_config['model'], prompt=prompt, temperature=temperature, top_p=top_p, disable_thinking=model_config['disable_thinking'])
...
def run_two_level(text: object, client: OpenAI, model_config: dict, temperature: float, top_p: float, request_interval: float, retries: int) -> tuple[str, Optional[float], str, str, Optional[float], str]:
    cleaned = clean_text(text)
    if cleaned == '':
        return ('Empty', None, 'Empty Content', 'Empty', None, 'Empty Content')
    prompt1 = PROMPT_LEVEL1.replace('{CORPUS}', cleaned)
    l1_label, l1_confidence, raw1 = call_prompt(prompt1, client, model_config, temperature, top_p, request_interval, retries)
    confidence_for_prompt = str(int(l1_confidence * 100)) if l1_confidence is not None else 'NA'
    prompt2 = PROMPT_LEVEL2.replace('{PRIMARY_L1}', l1_label).replace('{CONF_L1}', confidence_for_prompt).replace('{CORPUS}', cleaned)
    l2_label, l2_confidence, raw2 = call_prompt(prompt2, client, model_config, temperature, top_p, request_interval, retries)
    return (l1_label, l1_confidence, raw1, l2_label, l2_confidence, raw2)

def process_all(texts: list[object], client: OpenAI, model_config: dict, temperature: float, top_p: float, request_interval: float, max_workers: int, retries: int) -> list[tuple[str, Optional[float], str, str, Optional[float], str]]:
    results: list[Optional[tuple[str, Optional[float], str, str, Optional[float], str]]] = [None] * len(texts)
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(run_two_level, text, client, model_config, temperature, top_p, request_interval, retries): idx for idx, text in enumerate(texts)}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run self-reflective validation.')
...
def main() -> None:
    ...
    df_run = filter_by_rank(df, use_rank_filter=not args.no_rank_filter, rank_values=parse_rank_values(args.rank_values), rank_range=parse_rank_range(args.rank_range))
    ...
    for run_id in range(1, args.n_runs + 1):
        ...
        df[f'Pred_L1_run{run_id}'] = ''
        df[f'Confidence_L1_run{run_id}'] = np.nan
        df[f'Pred_L2_run{run_id}'] = ''
        df[f'Confidence_L2_run{run_id}'] = np.nan
        df.loc[df_run.index, f'Pred_L1_run{run_id}'] = [item[0] for item in preds]
        df.loc[df_run.index, f'Confidence_L1_run{run_id}'] = [item[1] for item in preds]
        df.loc[df_run.index, f'Pred_L2_run{run_id}'] = [item[3] for item in preds]
        df.loc[df_run.index, f'Confidence_L2_run{run_id}'] = [item[4] for item in preds]
        y_pred_by_run.append(df.loc[df_run.index, f'Pred_L2_run{run_id}'].tolist())
