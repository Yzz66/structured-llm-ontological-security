...
def build_pipeline(seed):
    return Pipeline([('tfidf', TfidfVectorizer(ngram_range=TFIDF_NGRAM_RANGE, min_df=TFIDF_MIN_DF, max_df=TFIDF_MAX_DF)), ('clf', LinearSVC(random_state=seed))])
...
def main():
    ...
    with txt_path.open('w', encoding='utf-8') as f:
        f.write('SVM Train/Test Evaluation\n')
        f.write('=' * 70 + '\n\n')
    ...
    for i, pred in enumerate(predictions, start=1):
        pred_df[f'Pred_Label_Run_{i}'] = pred
        pred_df[f'Correct_Run_{i}'] = (pred_df[args.label_col].astype(str) == pred_df[f'Pred_Label_Run_{i}'].astype(str)).astype(int)
        pred_df[f'Error_Type_Run_{i}'] = np.where(pred_df[f'Correct_Run_{i}'] == 1, '', pred_df[args.label_col].astype(str) + ' -> ' + pred_df[f'Pred_Label_Run_{i}'].astype(str))
    pred_df['Pred_Label'] = last_pred
    ...
    pred_path = output_dir / f'SVM_train_test_predictions_{timestamp}.xlsx'
